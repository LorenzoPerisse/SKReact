...
""" 
    Ver v2.1.0 (updated by Lorenzo Perisse)
    ...
    for file_name in file_names:
        ...
        for index, data in react_dat.iterrows():
            ...
            try:
                ...
                data_elevation = float(data[4])
                data_core_type = str(data[5]).strip()
                data_mox       = bool(data[6])
                # data_p_th    = pd.Series(float(data[7]),index=file_year)
                data_p_th      = float(data[7])
                # data_lf = []
                # for month in range(1, 13):
                #     data_lf.append(float(data[8 + month]))
            except ValueError:
            ...
            for reactor in reactors:
                if reactor.name == data_name:
                    ...
                    if data_elevation != reactor.elevation:
                        print(
                            "Reactor elevation has changed in file " + file_name + "!"
                        )
                        print("Reactor: " + reactor.name)
                        print(str(reactor.elevation) + " -> " + str(data_elevation))
                        print("Updating...")
                        reactor.elevation = data_elevation
                    ...
                    for month in range(1, 13):
                        ...
                        try:
                            # 7 is to skip the reactor data
                            reactor.add_to_lf(lf_header, float(data[7 + month]))
                        except:
                            if VERBOSE_IMPORT_ERR:
                                print(
                                ...
                                print("Load factor entry: %s" % data[7 + month])
                                # input("Press Enter to continue importing...")
                                print("Adding zeros...")
            ...
            if not in_reactors:
                ...
                for month in range(1, 13):
                    lf_header = file_year + "/%02i" % month
                    try:
                        reactors[-1].add_to_lf(lf_header, float(data[7 + month]))
                    except:
                        if VERBOSE_IMPORT_ERR:
                            print(
                                "Load factor data for "
...
total_osc_pre_factor  = pd.Series(dtype="float64")  # Oscillation factor
...
def main():
    ...
    for date, lf in reactors[0].lf_monthly.items():
        ...
        for reactor in reactors:
            ...
            reactor.n_ints_monthly.append(np.sum(int_spec)*E_INTERVAL)
        # smear_spec = wit_smear.smear(this_month_tot_spec)
        # monthly_ints.append(np.sum(this_month_tot_spec)*E_INTERVAL)
        year = int(date[:4])
        month = int(date[5:7])
    ...
    izu_genframe = ttk.Labelframe(rh_frame, text="Monthly Load Factor Generator")
    ...
    start_year_combo.set(file_year_start)
    ...
    def gen_monthlyoscfluxfile(*args):
        print("Generating monthly load factors")
    ...
    def update_n_nu(*args):
        ...
        osc_spec_ax.set_ylabel(r"$dN/dE$ [MeV^-1.cm^-2]")
        ...
        for highlighted_e_spec, spec_err in zip(highlighted_e_specs, spec_errs):
            ...
            e_spec_int += np.sum(highlighted_e_spec["Total"].tolist())*E_INTERVAL
        ...
        # Sum up all spectra
        total_osc_pre_factor = np.zeros(E_BINS)
        ...
        for reactor in reactors:
            start = time.time()
            osc_pre_factor = reactor.osc_pre_factor(dm_21=dm_21_val.get(), s_12=s_12_val.get(), period=period)
            osc_spec = reactor.osc_spec(dm_21=dm_21_val.get(), s_12=s_12_val.get(), period=period)
            end = time.time()
            ...
            start = time.time()
            # Sum of spectra
            total_osc_pre_factor += osc_pre_factor
            ...
            reactor.current_flux = np.sum(osc_spec)
        ...
        # Integrating using trap rule
        int_spec_int = np.sum(total_int_spec)*E_INTERVAL
        osc_spec_int = np.sum(total_osc_spec)*E_INTERVAL
        ...
        if smear_imported:
            ...
            det_spec_int = np.sum(smear_spec, dx=SMEAR_INTERVAL)
        ...
        # SAVE SPECTRUM
        # =================================================================
        # Save oscillated spectrum and oscillation prefactor to ROOT file
        # Only done once, controled by IsTFileUpdated
        global IsTFileUpdated
        if bool(IsTFileUpdated)==False:
            print('Writing oscillated prefactor to ROOT TFile.')
            
            # Make total spectrum at SK = superimposition of all reactor spectra
            for i in range(E_BINS):
                if ENERGIES[i]>=IBD_MIN:
                    content1 = total_osc_spec[i]
                    content2 = total_osc_pre_factor[i]
                    bin_i = hTot_SK.FindBin(ENERGIES[i])
                    hTot_SK.SetBinContent(bin_i,  content1  )
                    hTot_SK.SetBinError(i+1, 0.  )
                    oscillation_prefactor.SetBinContent(bin_i, content2 )

            inFile.cd()

            hTot_SK.GetXaxis().SetTitle("#nu kinetic energy  [MeV]")
            hTot_SK.GetYaxis().SetTitle("dN/dE  [#nu/MeV]")
            hTot_SK.Write()

            oscillation_prefactor.GetXaxis().SetTitle("#nu kinetic energy  [MeV]")
            oscillation_prefactor.GetYaxis().SetTitle("Oscillation prefactor")
            oscillation_prefactor.Write()

            inFile.Close()

            #Print reactor info
            # for reactor in reactors:
            #     osc_spec = reactor.osc_spec(period=("2020/01-2022/12"))
            #     int_spec = reactor.int_spec(osc_spec)
            #     # print(reactor.name, reactor.core_type, reactor.mox, reactor.p_monthly, int_spec)
            #     print(reactor.core_type, reactor.mox, reactor.p_monthly['2021/10'], np.sum(int_spec)*E_INTERVAL)

            IsTFileUpdated = True

            print("TFile updated and saved")
        ...
        loc_min = ticker.LogLocator(base=10.0, subs=(np.linspace(0.1, 0.9, 9).tolist()), numticks=10)
