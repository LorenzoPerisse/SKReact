...
def main():
    ...
    def update_n_nu(*args):
        ...
        # Print oscillation prefactor
        # print('Writing oscillated spectrum to file...')
        # file_total_osc_spec = open("Oscillated_spectrum.dat", "w")
        # file_total_osc_spec.write('E_nu(MeV)     Oscillation IBD spectrum(cm2)\n')
        # for i in range(E_BINS):
        #     if i>=1800 and i<8500:
        #         tmp_energy = (1.*i+0.5)/1000
        #         file_total_osc_spec.write('%f\t'%tmp_energy)
        #         file_total_osc_spec.write('%f\n'%total_int_spec[i])
        # file_total_osc_spec.close()
        ...
        if bool(IsTFileUpdated)==False:
            ...
            for i in range(E_BINS):
                if ENERGIES[i]>=IBD_MIN:
                    ...
                    if (i+1)%1000==0:
                        print(i+1, "/", E_BINS, " :  ", hTot_SK.GetBinCenter(bin_i), " MeV")

            inFile.cd()

            hTot_SK.GetXaxis().SetTitle("#nu kinetic energy  [MeV]")
            hTot_SK.GetYaxis().SetTitle("dN/dE  [#nu/MeV]")
            hTot_SK.SetTitle("Integrated #nu flux for the period: ")
            hTot_SK.Write()
